    ...
    <>
      ...
      <div className="mainPicAndDaddyBelowWrapper">
            ...
            <div className="sidePicsAndDaddyBelowWrapper">
              {spotsForDisplay && spotsForDisplay?.length > 1 && spotsForDisplay.slice(startCarousel, startCarousel + 4).map((ele, index) => {
                if (ele.preview === false) {
                  return (
                    <div className={`sidePieceHolder ${index === selectedImage ? 'selectedImage' : ''}`} onClick={() => {
        ...
        <div>
          ...
          <button onClick={(e) => {
            setStartCarousel(prevStart => spotsForDisplay[prevStart + 4] ? (prevStart + 4) : prevStart)
          }} disabled={startCarousel + 4 >= spotsForDisplay.length}>Right</button>
        </div>
